...
def init_particles_random(num_particles, occupancy_map):
    ...
    x0_vals = np.random.uniform(0, 7000, (num_particles, 1))
...
def init_particles_freespace(num_particles, occupancy_map):
    ...
    # get the locations where the occupancy map cell value is 0
    x_locs, y_locs = np.where(occupancy_map == 0)

    # pick random indices to select from
    idx_grid = np.arange(x_locs.shape[0])
    # sample without replacement
    idx = np.random.choice(idx_grid, num_particles, replace = False)

    # get final coords
    y0_vals, x0_vals = x_locs[idx].reshape((num_particles, 1)) * 10, y_locs[idx].reshape((num_particles, 1)) * 10
    # get theta vals
    theta0_vals = np.random.uniform(-3.14, 3.14, (num_particles, 1))
...
if __name__ == '__main__':
    ...
    for time_idx, line in enumerate(logfile):
        ...
        for m in range(0, num_particles):
            ...
            """
            if (meas_type == "L"):
                # z_t -> range finder measurnments for 0-180
                z_t = ranges
                # xt1 -> current estimate of state, x, y, theta
